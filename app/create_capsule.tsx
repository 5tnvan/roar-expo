...
import { insertCapsule } from "@/utils/supabase/insertCapsule";
...
export default function ModalCreateCapsule() {
  ...
  const handleGenerateContent = async () => {
    ...
    const promptText = `
    You are an AI assistant that generates a message content.

    Message: ${content ? content : "empty message"}

    If there is no message, generate a new random message.
    If there is message, generate new message based on given message. The first 350 characters should be a catchy preview, enticing users to read more.

    Return content only, no other comments.
    `;
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      alert("Capsule published successfully!");
    } catch (err) {
  ...
  return (
    <>
      {/* Modals */}
      <Preview visible={showPreviewModal} title={title} content={content} pdf_uri={pdfFile?.uri} pdf_content={pdfFile?.content} img_uri={finalMascot || ''} onClose={() => setshowPreviewModal(false)} />
      <PdfViewer visible={showPdfModal} onClose={() => setShowPdfModal(false)} pdfUri={pdfFile?.uri} />
      <ThemedView className="relative h-full">
        {/* Preview + Submit */}
        <View className="submit flex flex-row w-full gap-2 pt-3 px-2">
          <View className="flex-1">
            <AppButton
              title="Preview"
              variant="dark"
              size="lg"
              onPress={handlePrePublish}
            />
          </View>
          <View className="flex-1">
            <AppButton
        ...
        {/* ScrollView */}
        <ScrollView className="scroll-view flex flex-col p-2">
          {/* Dancing Pills */}
          <View className="dancing-pills my-1">
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {/* + Choose button */}
              <TouchableOpacity
                onPress={handleChooseMascotGif}
                className="mr-2 w-16 h-16 items-center justify-center border-2 border-dashed border-gray-400 rounded-md"
              >
                <Ionicons name="add" size={24} color="gray" />
              </TouchableOpacity>

              {/* Custom uploaded mascot first */}
              {customMascot && (
                <TouchableOpacity
          ...
          {uploadPdfMode &&
            (
              <View className="upload-pdf flex-1 py-2">
                <Pressable onPress={handlePdfExtract} className={`rounded-lg p-6 items-center justify-center border border-dashed ${isDark ? 'border-blue-600 bg-zinc-900' : 'border-blue-400 bg-zinc-50'}`}>
                  <View className="flex flex-row justify-center items-center gap-1">
                    <Ionicons name="document" size={22} color={isDark ? "white" : "black"} />
                    <Text className={`${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Tap to choose file</Text>
                  </View>
                  <View className="flex flex-row gap-1">
                    {!pdfFile ?
                      <TouchableOpacity onPress={handlePdfExtract} className="mt-2 bg-blue-500 px-4 py-2 rounded-lg">
                        <Text className="text-white">Upload PDF</Text>
                      </TouchableOpacity> :
                      <TouchableOpacity onPress={() => setPdfFile(null)} className="mt-2 bg-blue-500 px-4 py-2 rounded-lg">
                        <Text className="text-white">Remove PDF</Text>
                      </TouchableOpacity>}

                    {pdfFile && (
                      <TouchableOpacity onPress={() => setShowPdfModal(true)} className="mt-2 bg-blue-500 px-4 py-2 rounded-lg">
                  ...
                  >
                    ...
                    <Text className={`${isDark ? 'text-gray-500' : 'text-gray-500'}`}>{pdfFile.charCount} characters</Text>
                  </>}
                </Pressable>
              </View>
            )}
          ...
          {/* Message */}
          <View className="message flex-1 pt-1">
            <TextInput
              ...
              multiline
              style={{ fontSize: 18, padding: 10, textAlignVertical: "top" }} // bigger text
              className={`border border-blue-500 rounded-lg flex-1 ${isDark ? "bg-black text-white" : "bg-white text-black"
                } h-96`} // h-36 roughly 6 lines
            />
          </View>
        </ScrollView>
      </ThemedView>
      <SafeAreaView className={`absolute bottom-0 w-full ${isDark ? "bg-transparent" : "bg-transparent"}`}>
        {/* Inspo */}
        <View className="inspo">
          <ScrollView
            horizontal
          ...
          >
            {inspirations.map((item, idx) => (
              <TouchableOpacity
                ...
                className="rounded-lg w-36 border border-gray-500/50 p-4 mr-2"
              >
                <Ionicons
        ...
        {/* Menu */}
        <View className="extra-menu w-full flex flex-row gap-3 pt-2 px-2">
          <TouchableOpacity
          ...
          <TouchableOpacity
            ...
            onPress={() => setUploadPdfMode(prev => !prev)} // toggle instead of forcing true
          >
            <Ionicons
              name="document"
              size={18}
            ...
            />
            <Text
              className={`${isDark ? "text-blue-500" : "text-blue-500"
                } text-lg truncate max-w-[200px]`}
              numberOfLines={1}
            >
              {pdfFile?.name || "PDF"}
