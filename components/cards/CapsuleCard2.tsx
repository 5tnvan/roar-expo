...
import BlurButton1 from "../buttons/BlurButton1";
    ...
    <View className={`rounded-xl overflow-hidden relative ${isDark ? "bg-zinc-900" : "bg-white"} shadow-md`}>
      ...
      {capsule.image_url && (
        <View className="">
          <Image
            ...
            className="w-full h-96 rounded-t-xl" // round top corners
            resizeMode="cover"
          />
          <View className="absolute w-full top-1/2 -translate-y-1/22">
            <BlurButton1
              readMinutes={readMinutes as number}
              onPress={() => onReadWithAI && onReadWithAI(capsule)}
            />
          </View>
        </View>
      )}
      ...
      <View className="px-4 pt-2 pb-4">
        ...
        <View className="flex-row items-center justify-between mb-3">
          <Link href={`/profile/${capsule.owner.id}`} className="">
            <View className="flex-row items-center gap-2">
              <Avatar uri={capsule.owner.avatar_url} size={40} showTick plan={capsule.owner.plan} />
              <Text className={`font-normal text-lg ${isDark ? "text-white" : "text-black"}`}>
                <TruncatedText text={`@${capsule.owner.handle}`} maxLength={18} />
              </Text>
